<template>
  <div class="payment-misuse-config">
    <el-form 
      :model="formData" 
      :disabled="disabled"
      label-width="120px"
    ...
    >
      ...
      <el-form-item label="预警规则">
        <div class="rule-description">
          <el-alert
            title="如果施工单位在代发资金模块进行对公账户付款，则进行预警"
            type="warning"
            :closable="false"
            show-icon
          />
      ...
      <el-form-item label="处置方式">
        ...
        <div class="processing-description">
          <p v-if="formData.processingType === 'viewOnly'">
            ...
            该代发提交命中预警规则时，业主方首页弹出预警信息，提交支取时弹出预警信息
          </p>
...
import type { PaymentMisuseWarningConfig, WarningLevel, ProcessingType } from '../../types/warning'

interface Props {
  config: PaymentMisuseWarningConfig
  disabled?: boolean
  editMode?: boolean
}

interface Emits {
  (e: 'update', config: PaymentMisuseWarningConfig): void
}
...
const emitUpdate = () => {
  const updatedConfig: PaymentMisuseWarningConfig = {
    ...props.config,
